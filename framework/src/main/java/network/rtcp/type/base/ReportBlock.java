...
public class ReportBlock {
    ...
    private int dlsr; // (32 bits)

    ////////////////////////////////////////////////////////////

    ////////////////////////////////////////////////////////////
    // CONSTRUCTOR
    public ReportBlock(int ssrc, byte f, int c, int ehsn, int j, int lsr, int dlsr) {
        this.ssrc = ssrc;
        this.f = f;
        this.c = c;
        this.ehsn = ehsn;
    ...
    public byte[] getByteData() {
        ...
        byte[] ssrcData = ByteUtil.intToBytes(ssrc, true);
        System.arraycopy(ssrcData, 0, data, index, ssrcData.length);
        index += ssrcData.length;

        byte[] fcData = new byte[4];
        byte[] fData = { f };
        System.arraycopy(fData, 0, fcData, 0, 1);
        byte[] cData = ByteUtil.intToBytes(c, true);
        System.arraycopy(cData, 0, fcData, 1, 3);
        System.arraycopy(fcData, 0, data, index, fcData.length);
        index += fcData.length;

        byte[] ehsnData = ByteUtil.intToBytes(ehsn, true);
        System.arraycopy(ehsnData, 0, data, index, ehsnData.length);
        index += ehsnData.length;

        byte[] jData = ByteUtil.intToBytes(j, true);
        System.arraycopy(jData, 0, data, index, jData.length);
        index += jData.length;

        byte[] lsrData = ByteUtil.intToBytes(lsr, true);
        System.arraycopy(lsrData, 0, data, index, lsrData.length);
        index += lsrData.length;

        byte[] dlsrData = ByteUtil.intToBytes(dlsr, true);
        System.arraycopy(dlsrData, 0, data, index, dlsrData.length);

        return data;
    }

    public void setData(int ssrc, byte f, int c, int ehsn, int j, int lsr, int dlsr) {
        this.ssrc = ssrc;
        this.f = f;
        this.c = c;
        this.ehsn = ehsn;
